...
describe("bookings rules", () => {
  ...
  it("người lạ không đọc được vé của người khác", async () => {
  ...
  it("KẺ TẤN CÔNG: hồi sinh vé đã hủy (status: cancelled -> confirmed) -> bị chặn", async () => {
    const alice = env.authenticatedContext("alice");
    await assertFails(
      setDoc(doc(alice.firestore(), "bookings/bk1"), {
        userId: "alice",
        eventId: "pub1",
        status: "confirmed",
        totalAmount: 0,
...
describe("buyers rules", () => {
  ...
  it("đọc counter của người khác -> bị chặn", async () => {
    const bob = env.authenticatedContext("bob");
  ...
  it("KẺ TẤN CÔNG: tự reset bookingCount về 0 -> bị chặn", async () => {
    const alice = env.authenticatedContext("alice");
    await assertFails(
      setDoc(doc(alice.firestore(), "events/pub1/buyers/alice"), {
        bookingCount: 0,
      }),
    );
  });
